...
export default function RehearsalDinnerPage() {
  ...
  function printDinner() {
    ...
    const timelineHtml = data.timeline.length > 0
      ? data.timeline.map((t) => `<div style="display:flex;gap:12px;padding:6px 0;border-bottom:1px solid #E8D5B7"><span style="font-weight:600;color:#2C3E2D;min-width:80px">${t.time}</span><span style="color:#1A1A2E">${t.event}</span></div>`).join("")
      : '<p style="color:#8E7A9E">No timeline items</p>';

    const guestHtml = data.guest_list.length > 0
      ? data.guest_list.map((g) => `<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #E8D5B7"><span style="color:#1A1A2E">${g.name}</span><span style="color:#8E7A9E;font-size:12px">${RSVP_LABELS[g.rsvp] || g.rsvp}</span></div>`).join("")
    ...
    printWin.document.write(`<!DOCTYPE html><html><head><title>Rehearsal Dinner</title><style>body{font-family:system-ui,sans-serif;padding:40px;color:#1A1A2E}@media print{body{padding:20px}}</style></head><body>
      ...
      ${data.hosted_by ? `<p style="color:#8E7A9E;font-size:14px">Hosted by ${data.hosted_by}</p>` : ""}
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:16px 0;font-size:14px">
        ...
        ${data.dress_code ? `<div><strong>Dress Code:</strong> ${data.dress_code}</div>` : ""}
      </div>
      ${data.notes ? `<p style="font-size:13px;color:#6B6B6B;margin:16px 0">${data.notes}</p>` : ""}
