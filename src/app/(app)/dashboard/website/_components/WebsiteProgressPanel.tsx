...
export function WebsiteProgressPanel({ summary, onJumpToTab }: WebsiteProgressPanelProps) {
  const { stageInfo, outstanding, doneCount, totalCount, overdueCount, isComplete } = summary;
      ...
      {isComplete ? (
        ...
        <div className="mt-4">
          <p className="text-[11px] font-semibold text-muted uppercase tracking-wide mb-2">
            {overdueCount > 0 ? `Needs attention — ${overdueCount} overdue` : "Still to do"}
          </p>
          <div className="grid sm:grid-cols-2 gap-1">
            {outstanding.map((item) => (
              ...
              >
                <span
                  className={`flex-shrink-0 w-[15px] h-[15px] rounded-full border-2 ${
                    item.overdue ? "border-amber-500" : "border-violet/40"
                  }`}
                  aria-hidden="true"
                />
                <span className="text-[14px] text-plum group-hover:text-violet transition">
                  {item.label}
                </span>
                {item.overdue && (
