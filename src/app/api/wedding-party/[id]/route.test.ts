...
function patchReq(body: unknown): Request {
  return new Request("http://localhost/api/wedding-party/wp1", {
    method: "PATCH",
...
describe("PATCH /api/wedding-party/[id]", () => {
  it("forwards auth errors", async () => {
    mockGetWeddingForUser.mockResolvedValue({
      error: NextResponse.json({ error: "Unauthorized" }, { status: 401 }),
    });
    const res = await PATCH(patchReq({ name: "x" }), ctx("wp1") as never);
    expect(res.status).toBe(401);
  });

  it("returns 400 when no allowed fields are provided", async () => {
    const res = await PATCH(patchReq({ notARealField: 1 }), ctx("wp1") as never);
    expect(res.status).toBe(400);
  });

  it("updates allowed fields and returns the updated row", async () => {
    const res = await PATCH(
      patchReq({ name: "Alice", role: "Maid of Honor", sort_order: 1 }),
      ctx("wp1") as never
    );
  ...
  it("silently drops unknown fields while accepting valid ones", async () => {
    const res = await PATCH(
      patchReq({ name: "Alice", hackerField: "pwned" }),
      ctx("wp1") as never
    );
    expect(res.status).toBe(200);
  });
...
describe("DELETE /api/wedding-party/[id]", () => {
  it("forwards auth errors", async () => {
    mockGetWeddingForUser.mockResolvedValue({
      error: NextResponse.json({ error: "Unauthorized" }, { status: 401 }),
    });
    const res = await DELETE(new Request("http://localhost"), ctx("wp1") as never);
    expect(res.status).toBe(401);
  });
