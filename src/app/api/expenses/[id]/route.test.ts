...
function patchReq(body: unknown): Request {
  return new Request("http://localhost/api/expenses/exp_1", {
    method: "PATCH",
...
beforeEach(() => {
  ...
  mockUpdateSingle.mockResolvedValue({
    data: { id: "exp_1", description: "Flowers" },
    error: null,
  });
...
describe("PATCH /api/expenses/[id]", () => {
  it("forwards auth errors", async () => {
    ...
    const res = await PATCH(patchReq({ description: "x" }), ctx("exp_1") as never);
    expect(res.status).toBe(401);
  });

  it("returns 400 when no allowed fields are provided", async () => {
    const res = await PATCH(patchReq({ notARealField: 1 }), ctx("exp_1") as never);
    expect(res.status).toBe(400);
  });

  it("updates allowed fields and returns the updated row", async () => {
    const res = await PATCH(
      patchReq({ description: "Venue", estimated: 5000 }),
      ctx("exp_1") as never
    );
...
describe("DELETE /api/expenses/[id]", () => {
  it("forwards auth errors", async () => {
    ...
    const res = await DELETE(new Request("http://localhost"), ctx("exp_1") as never);
    expect(res.status).toBe(401);
  });
